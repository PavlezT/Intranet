...
export class Birthdays {
  ...
  private getBirthUsers(target : string) : Promise<any> {
    let url = `${consts.siteUrl}/_api/web/lists('${this.guid}')/GetItems(Query=@target)?$select=UserEmail,LSiBirthdayD,LSiBirthdayM,Title,Id,User1Id,JobTitle&$top=500&@target={"ViewXml":"${target}"}`;
