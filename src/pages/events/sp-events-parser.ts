...
export class spEventsParser {

	constructor(){}

		public parseEvents(events,start,end){
			var full = [];
			...
			if(!e.fRecurrence){
				...
				if(e.RecurrenceData.indexOf("<daily ")!=-1){
					...
					if(arr.indexOf("dayFrequency") != -1){
						...
						while(loop){
							if((new Date(init) > end) || (rTotal > 0 && rTotal <= total)) {loop = false;continue; }
							total++;
